...
F_kap = 1;      % cap kappa to avoid poor accuracy (1) or not (0)
...
%% SIMULATION(S) %%%%%%%%%%%%%%%%%%%%%
% 'exact solution' with high sampling frequency
FsES = 256*44100;
dtES = 1/FsES;
NsES = ceil(dur*FsES);
tES = (0:(NsES-1))*dtES;
...
h3 = plot(1000*t,1000*outp.x,'--','LineWidth',LW);
h4 = plot(1000*t,1000*outp.y,'--','LineWidth',LW);
hold off;
grid;
...
hold on;
plot(1000*t,outp.Fc,'--','LineWidth',LW);
hold off;
grid;
ylabel('F_{c} (N)');
...
hold on;
plot(1000*t,outp.Qc,'--','LineWidth',LW);
hold off;
xlabel('time (ms)');
ylabel('Q_{c} (W)');
grid;
...
hold on;
plot(1000*t,outp.H,'--','LineWidth',LW);
hold off;
xlabel('time (ms)');
ylabel('H (J)');
grid;
